...
function zume_site_scripts() {
    ...
    zume_enqueue_script( 'base-js', 'assets/scripts/base.js', array( 'jquery' ), true );
    wp_localize_script(
        "base-js", "zumeBase", array(
            ...
            "translations" => array(
                ...
                'course' => array(
                    ...
                    "current:" => __( 'Current Step:', 'zume' ),
                    "pagination" => __( 'Cancel', 'zume' ),
                    "finish" => __( 'Finish', 'zume' ),
                    "next" => __( 'Next', 'zume' ),
                    "previous" => __( 'Previous', 'zume' ),
                    "loading" => __( 'Loading...', 'zume' ),
                ),
                'profile' => [
    ...
    if ( 'template-zume-get-a-coach.php' === basename( get_page_template() ) ) {
        wp_register_script( 'lodash', 'https://cdnjs.cloudflare.com/ajax/libs/lodash.js/4.17.11/lodash.min.js', false, '4.17.11' );
        wp_enqueue_script( 'lodash' );
        wp_enqueue_script( 'zume-profile', get_template_directory_uri() . '/assets/scripts/coaching-request.js', array( 'jquery', 'lodash', 'wp-i18n', 'base-js' ), filemtime( get_theme_file_path() . '/assets/scripts/coaching-request.js' ), true );
    }

    if ( 'template-zume-course.phpold' === basename( get_page_template() ) /* 3.0 */ || 'template-zume-course-v4.php' === basename( get_page_template() ) /* 4.0 */ ) {
    ...
    if ( 'template-zume-profile.phpold' === basename( get_page_template() ) ) {
...
//                ],
    ...
    if ( 'template-zume-training.php' === basename( get_page_template() ) ) {
        wp_register_script( 'lodash', 'https://cdnjs.cloudflare.com/ajax/libs/lodash.js/4.17.11/lodash.min.js', false, '4.17.11' );
        wp_enqueue_script( 'lodash' );
        wp_enqueue_script( 'zume-profile', get_template_directory_uri() . '/assets/scripts/training-page.js', array( 'jquery', 'lodash', 'wp-i18n', 'base-js' ), filemtime( get_theme_file_path() . '/assets/scripts/training-page.js' ), true );
    }

    /**
     Zume 4.0
     */
    if ( 'template-zume-training.phpold' === basename( get_page_template() )
        || 'template-pieces-page.php' === basename( get_page_template() )
        || 'template-zume-course-v4.phpold' === basename( get_page_template() ) ) {
        wp_register_script( 'lodash', 'https://cdnjs.cloudflare.com/ajax/libs/lodash.js/4.17.11/lodash.min.js', false, '4.17.11' );
        wp_enqueue_script( 'lodash' );
...
//                ],
                ...
                "zume_network_sites" => ( is_user_logged_in() ) ? zume_network_sites() : array(),
                'urls' => array(
                    1 => esc_url( zume_get_landing_translation_url( 1, $current_language ) ),
                    2 => esc_url( zume_get_landing_translation_url( 2, $current_language ) ),
                    3 => esc_url( zume_get_landing_translation_url( 3, $current_language ) ),
